...
- 未来如果出现 RSS 等更多异步模块，再提取为独立包也不迟（模块边界清晰，迁移成本低）
...
定时任务和 RSS 订阅表面相似（都是"触发→推送"），但实际差异显著：
...
各模块独立实现自己的触发、执行、状态管理逻辑，通过共享底层能力协作。当未来真正有 3 个以上类似模块、且发现自己在重复写相同的生命周期管理代码时，再提取公共层。
