...
export function RssSettingsPanel() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      <header>
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex min-w-0 flex-col gap-1">
            ...
            <p className="text-sm leading-5 text-muted">管理 RSSHub 地址、认证方式和抓取超时。</p>
          </div>
      ...
      {error ? <ErrorNotice>加载 RSS 设置失败：{error}</ErrorNotice> : null}
          ...
          {authType === "bearer" ? (
            <div className="flex flex-col gap-2.5">
              ...
              {settings?.rsshub_bearer_token_set ? (
                <div className="flex items-center justify-between gap-3 text-sm text-muted-strong">
                  <span>{clearBearerToken ? "保存后将清空已保存 Token" : "当前已保存 Token"}</span>
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => {
