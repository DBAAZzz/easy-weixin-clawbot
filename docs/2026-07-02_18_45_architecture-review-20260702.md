...
- **M1. 单进程假设未显式化**：in-memory history 锁、seq 计数、sessionCache、scheduler 全部假设单实例。现在可接受，但应写进 `docs/2026-04-01_17_39_agent-architecture.md`，避免有人起两个实例（会双倍回复 + seq 冲突丢数据）。成本 S。
...
5. 单进程约束、启动顺序、port 注入图写入 `docs/2026-04-01_17_39_agent-architecture.md`。
...
## 附：E 节 / D 阶段 3 第 1、2 项实施记录（2026-07-24）

对应 [模块边界重构方案](./2026-07-24_15_37_agent-module-boundary-refactor.md)，落地了 E 节「模块边界（agent 包内）」的 `engine/` / `capabilities/` / `memory/` / `ports/` 划分，以及 D 阶段 3 的第 1 项（`history.ts` 五个全局 Map 收进可实例化的 `ConversationCache`）与第 2 项（`server/agent.ts` 的部分职责梳理——但**不包括**完整拆分，H5 本次显式排除在外，只做了调用点从 `chat()`/自由函数切换到 `chatEngine.*` 方法）。

**做了什么**：
- 物理重组：`src/` 98 个文件按 L5(`engine`) → L4(`capabilities`) → L3(`memory`) → L2(`llm`/`prompts`/`commands`) → L1(`ports`) → L0(`shared`) 分层落位，`git mv` 保留文件历史；`package.json` exports 的 key 不变，value 指向新路径，server 侧 import 说明符零改动。
- 去全局化：`RunContext`（`engine/context.ts`）替代 `chat()` 的 6 个位置参数；`ConversationCache`（`engine/conversation/cache.ts`）替代 `history.ts` 的 5 个模块级 Map；`ChatEngine`（`engine/chat-engine.ts` + `engine/turn.ts`）替代 `chat.ts` 的 `_deps`/`setChatDeps`；`DebugFlags`（`commands/debug.ts`）替代 `debugState` Map。`engine/` 目录现在零模块级可变状态。
- 新增 `ChatExecutorPort`（`ports/chat-executor.ts`），把已有的 `HeartbeatExecutorPort` 模式泛化，同时服务 scheduler 与 heartbeat，消灭了 `scheduler/executor.ts` 直接 import `engine/chat.ts` + `engine/conversation/history.ts` 的、评审文档 E 节点名的**唯一真正架构违规**。
- `packages/agent/scripts/check-layers.mjs` + `lint:layers` script：用工具而非公约执行分层规则（`import type` 豁免，值导入必须向下）。
- 新增 `test/engine/conversation/cache.test.ts`、`test/engine/chat-engine.test.ts`、`test/capabilities/scheduler/executor.test.ts`——这是 H3 点名的"最复杂、并发最敏感的代码恰好零测试"里，`history.ts`/`chat.ts`/`scheduler/executor.ts` 三处第一次有了单测，全部用假 port 构造隔离实例跑，不依赖全局状态。

**没做的**（保持原计划边界，留给后续）：
- H2 账号级工具权限过滤、H1 消息持久化分队列、C4 密钥加密——与本次物理重组正交，未触碰。
- H5 `server/agent.ts` 职责拆分（pre-chat hooks / dispatchCommand / runChat / post-chat hooks 四函数化）——本次只做了"调用点从自由函数换成 `chatEngine.*` 方法"这一件事，`createAgent` 内部仍是原来的单函数结构，故意不在同一个 diff 里做两件事。
- M1 单进程假设：已按计划显式化，写进 `docs/2026-04-01_17_39_agent-architecture.md` 附录与 `packages/agent/AGENTS.md`，四处允许的进程级单例（`model-resolver` 缓存、`memory/queue.ts`、`heartbeat/engine.ts`、`scheduler/manager.ts`）原样保留，未做多实例改造。

**验证**：`pnpm -F @clawbot/agent exec tsc --noEmit`、`pnpm -F @clawbot/agent exec tsc --noEmit -p test/tsconfig.json`、`pnpm -F @clawbot/server exec tsc --noEmit`、`pnpm test:agent`、`pnpm test:server`、`pnpm -F @clawbot/agent lint:layers`、`pnpm build` 全部通过。端到端人工验证（发消息、`/debug`、`/reset`、定时任务、heartbeat act、模型热更新）见下方 checklist，由本次改动的执行者在合并前手动跑一遍。
